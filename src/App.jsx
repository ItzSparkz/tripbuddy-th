...
import { 
  ...
  BarChart2, Users, AlertCircle, Database, CalendarCheck, Clock
} from 'lucide-react';

// --- DATABASE 77 PROVINCES (COMPLETE) ---
const THAILAND_DATA = {
...
const INITIAL_POSTS = [
  { id: 1, title: '🔥 โปรฯ ภูเก็ต 3 วัน 2 คืน (พักศรีพันวา)', location: 'ภูเก็ต', author: 'ไกด์สมศรี', type: 'trip', media: 'https://lh3.googleusercontent.com/p/AF1QipPi0cGcFucBS2IJrzKVgt8iArNRfsvJGqoGCEvY=s1360-w1360-h1020-rw', chat: [], likes: 342, price: 4990, participants: [] },
...
const INITIAL_TRANSACTIONS = [
  { id: 101, from: 'นักเดินทาง Alex', to: 'ไกด์สมศรี', amount: 4990, date: '2024-02-15', status: 'pending', slip: 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=200', postId: 1 }
];
...
const Badge = ({ status }) => {
  const styles = { verified: 'bg-green-100 text-green-700', pending: 'bg-yellow-100 text-yellow-700', active: 'bg-blue-100 text-blue-700', rejected: 'bg-red-100 text-red-700' };
  return <span className={`px-2 py-0.5 rounded-full text-xs font-bold uppercase ${styles[status] || 'bg-gray-100'}`}>{status}</span>;
};

// --- COMPONENT: TOURISM INSIGHTS (STATS) ---
  ...
  <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8 animate-in fade-in slide-in-from-bottom duration-500">
    <div className="flex flex-col md:flex-row gap-8">
      <div className="flex-1 space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-800 flex items-center gap-2 mb-2"><Compass className="text-blue-600 w-8 h-8"/> เกี่ยวกับ TripbuddyTH</h2>
          <p className="text-gray-500 leading-relaxed">TripbuddyTH คือคอมมูนิตี้สำหรับคนรักการท่องเที่ยวที่ครบวงจรที่สุด เชื่อมต่อนักเดินทางและไกด์ท้องถิ่นเข้าด้วยกัน</p>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-blue-50 p-4 rounded-xl text-center"><Users className="w-6 h-6 text-blue-600 mx-auto mb-2"/><div className="text-2xl font-bold text-blue-800">12K+</div><div className="text-xs text-blue-600">ผู้ใช้งาน</div></div>
          <div className="bg-green-50 p-4 rounded-xl text-center"><Map className="w-6 h-6 text-green-600 mx-auto mb-2"/><div className="text-2xl font-bold text-green-800">850+</div><div className="text-xs text-green-600">ทริป</div></div>
          <div className="bg-orange-50 p-4 rounded-xl text-center"><Star className="w-6 h-6 text-orange-600 mx-auto mb-2"/><div className="text-2xl font-bold text-orange-800">4.8</div><div className="text-xs text-orange-600">รีวิว</div></div>
        </div>
      </div>
      <div className="flex-1 bg-gray-50 rounded-xl p-5 border border-gray-100">
        <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><BarChart2 className="w-5 h-5 text-indigo-600"/> 5 อันดับจังหวัดยอดฮิต</h3>
        <div className="space-y-4">
          {TOURISM_STATS.map((item, index) => (
            <div key={index} className="space-y-1">
              <div className="flex justify-between text-sm"><span className="font-medium text-gray-700 flex items-center gap-2"><span className={`w-5 h-5 rounded-full flex items-center justify-center text-xs text-white ${index < 3 ? 'bg-yellow-400' : 'bg-gray-400'}`}>{index + 1}</span>{item.province}</span><span className="text-gray-500 text-xs">{item.visitors}</span></div>
              <div className="h-2 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full ${item.color} rounded-full transition-all duration-1000 ease-out`} style={{ width: `${item.score}%` }}></div></div>
            </div>
          ))}
        </div>
      </div>
    ...
    <>
      ...
      <div className="fixed top-0 left-0 h-full w-72 bg-white z-[60] shadow-2xl animate-in slide-in-from-left duration-200 flex flex-col">
        ...
        <div className="p-4 space-y-2 flex-1">
          ...
          <button onClick={() => { setView('my_activity'); onClose(); }} className="w-full flex items-center gap-3 p-3 hover:bg-purple-50 rounded-xl text-gray-700 transition-colors"><div className="bg-purple-100 p-2 rounded-lg"><CalendarCheck className="w-5 h-5 text-purple-600" /></div> ประวัติของฉัน</button>
          <button className="w-full flex items-center gap-3 p-3 hover:bg-blue-50 rounded-xl text-gray-700 transition-colors"><div className={`p-2 rounded-lg ${user.status === 'verified' ? 'bg-green-100' : 'bg-gray-100'}`}><ShieldCheck className={`w-5 h-5 ${user.status === 'verified' ? 'text-green-600' : 'text-gray-500'}`} /></div> สถานะ: {user.status}</button>
        </div>
    ...
    <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
      ...
      {view === 'login' ? (
        <form onSubmit={handleLogin} className="space-y-4">
          <input required type="text" placeholder="ชื่อผู้ใช้ (admin / traveler1 / guide1)" className="w-full border p-3 rounded-lg bg-gray-50" value={loginForm.username} onChange={e => setLoginForm({...loginForm, username: e.target.value})} />
          <input required type="password" placeholder="รหัสผ่าน (123)" className="w-full border p-3 rounded-lg bg-gray-50" value={loginForm.password} onChange={e => setLoginForm({...loginForm, password: e.target.value})} />
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          {notification && <div className={`p-3 rounded-lg text-sm flex items-center gap-2 animate-in fade-in ${notification.type === 'error' ? 'bg-red-50 text-red-600' : 'bg-green-50 text-green-600'}`}>{notification.type === 'error' ? <XCircle className="w-4 h-4"/> : <CheckCircle className="w-4 h-4"/>}{notification.message}</div>}
          <div><label className="block text-sm font-medium text-gray-700 mb-1">ชื่อผู้ใช้</label><input required className="w-full border p-3 rounded-lg" onChange={e => setRegForm({...regForm, username: e.target.value})} /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">รหัสผ่าน</label><input required type="password" className="w-full border p-3 rounded-lg" onChange={e => setRegForm({...regForm, password: e.target.value})} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">ยืนยันรหัสผ่าน</label><input required type="password" className="w-full border p-3 rounded-lg" onChange={e => setRegForm({...regForm, confirmPassword: e.target.value})} /></div>
          </div>
          <div><label className="block text-sm font-medium text-gray-700 mb-1">ชื่อที่แสดงในระบบ</label><input required className="w-full border p-3 rounded-lg" onChange={e => setRegForm({...regForm, name: e.target.value})} /></div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">เลือกบทบาท</label>
            <div className="flex gap-2">
              <button type="button" onClick={() => setRegForm({...regForm, role: 'traveler'})} className={`flex-1 py-3 rounded-lg border flex items-center justify-center gap-2 ${regForm.role === 'traveler' ? 'bg-blue-50 border-blue-500 text-blue-700' : 'hover:bg-gray-50'}`}><User className="w-4 h-4"/> นักเดินทาง</button>
              <button type="button" onClick={() => setRegForm({...regForm, role: 'guide'})} className={`flex-1 py-3 rounded-lg border flex items-center justify-center gap-2 ${regForm.role === 'guide' ? 'bg-green-50 border-green-500 text-green-700' : 'hover:bg-gray-50'}`}><Map className="w-4 h-4"/> ไกด์</button>
            </div>
          </div>
    ...
    <div className="space-y-4">
      ...
      {tab === 'info' ? (
        <div className="space-y-3">
           ...
           <textarea className="w-full border p-2 rounded-lg h-20" value={formData.bio} onChange={e => setFormData({...formData, bio: e.target.value})} />
           <input className="w-full border p-2 rounded-lg" value={formData.contact} onChange={e => setFormData({...formData, contact: e.target.value})} placeholder="ติดต่อ" />
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <h3 className="font-bold text-lg mb-4 text-gray-700">ทริปที่เข้าร่วม ({myTrips.length})</h3>
          {myTrips.map(trip => (
            <div key={trip.id} className="bg-white p-4 rounded-xl border mb-3 shadow-sm flex gap-3">
              <div className="w-16 h-16 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0">{trip.type==='video'?<div className="w-full h-full bg-black flex items-center justify-center"><Video className="text-white w-6 h-6"/></div>:<img src={trip.media} className="w-full h-full object-cover"/>}</div>
              <div className="flex-1">
                <div className="font-bold line-clamp-1">{trip.title}</div>
                <div className="text-xs text-gray-500 mb-1">{trip.date || 'ไม่ระบุวันที่'} • {trip.location}</div>
                <Badge status={trip.participants.includes(user.name) ? 'active' : 'pending'} />
              </div>
            </div>
          ))}
        </div>
        <div>
          <h3 className="font-bold text-lg mb-4 text-gray-700">ประวัติการชำระเงิน ({myTrans.length})</h3>
          {myTrans.map(t => (
            <div key={t.id} className="bg-white p-4 rounded-xl border mb-3 shadow-sm flex justify-between items-center">
              <div><div className="font-bold">฿{t.amount.toLocaleString()}</div><div className="text-xs text-gray-500">{t.date} → {t.to}</div></div>
...
// --- ADMIN PANEL (ENHANCED DB VIEW) ---
const AdminPanel = ({ users, transactions, onVerifyUser, onDeleteUser, onApprovePayment }) => {
  const [tab, setTab] = useState('users');
  return (
    <div className="space-y-6">
      <div className="flex gap-2 overflow-x-auto pb-2"><button onClick={() => setTab('users')} className={`px-4 py-2 rounded-full ${tab === 'users' ? 'bg-blue-600 text-white' : 'bg-white border'}`}>ฐานข้อมูลลูกค้า</button><button onClick={() => setTab('verify')} className={`px-4 py-2 rounded-full ${tab === 'verify' ? 'bg-yellow-500 text-white' : 'bg-white border'}`}>คำขออนุมัติ</button><button onClick={() => setTab('payments')} className={`px-4 py-2 rounded-full ${tab === 'payments' ? 'bg-green-600 text-white' : 'bg-white border'}`}>การเงิน</button></div>
      {tab === 'users' && (
        <div className="bg-white rounded-xl border overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-gray-600"><tr><th className="p-4">ผู้ใช้งาน</th><th className="p-4">บทบาท</th><th className="p-4">สถานะ</th><th className="p-4">วันที่สมัคร</th><th className="p-4">จัดการ</th></tr></thead>
            <tbody className="divide-y">{users.map(u => (<tr key={u.id} className="hover:bg-gray-50"><td className="p-4 flex items-center gap-3"><img src={u.image||"https://ui-avatars.com/api/?name="+u.name} className="w-8 h-8 rounded-full"/><div className="font-bold">{u.name}</div></td><td className="p-4 capitalize">{u.role}</td><td className="p-4"><Badge status={u.status}/></td><td className="p-4">{u.joinedAt || '2024-02-01'}</td><td className="p-4">{u.role!=='admin'&&<button onClick={()=>onDeleteUser(u.id)} className="text-red-500 hover:text-red-700"><Trash2 className="w-4 h-4"/></button>}</td></tr>))}</tbody>
          </table>
        </div>
      )}
      {tab === 'verify' && (
        <div className="space-y-4">{users.filter(u => u.status === 'pending').map(u => (<div key={u.id} className="bg-white p-5 rounded-xl border-l-4 border-yellow-500 shadow-sm"><div className="flex items-center gap-3 mb-3"><img src={u.image || "https://ui-avatars.com/api/?name="+u.name} className="w-10 h-10 rounded-full"/><div><div className="font-bold">{u.name}</div><div className="text-xs text-gray-500">ขอเป็น: {u.role}</div></div></div><div className="bg-gray-50 p-3 rounded-lg text-sm text-gray-700 mb-4 italic">"{u.verifyRequest}"</div><div className="flex gap-2"><Button onClick={() => onVerifyUser(u.id, 'rejected')} variant="danger" className="flex-1 text-sm">ปฏิเสธ</Button><Button onClick={() => onVerifyUser(u.id, 'verified')} variant="success" className="flex-1 text-sm">อนุมัติ</Button></div></div>))}</div>
      )}
      {tab === 'payments' && (
        <div className="space-y-4">{transactions.filter(t => t.status === 'pending').map(t => (<div key={t.id} className="bg-white p-5 rounded-xl border-l-4 border-green-500 shadow-sm"><div className="flex justify-between items-start mb-3"><div><div className="font-bold text-gray-800">฿{t.amount.toLocaleString()}</div><div className="text-xs text-gray-500">จาก: {t.from} → ถึง: {t.to}</div></div><div className="text-xs bg-gray-100 px-2 py-1 rounded">{t.date}</div></div><Button onClick={() => onApprovePayment(t.id, t.from, t.postId)} variant="success" className="w-full text-sm">ยืนยันยอดเงิน</Button></div>))}</div>
      )}
    </div>
  );
};

// --- THAILAND DISCOVERY (FULL) ---
...
export default function App() {
  ...
  // Persistence
  useEffect(() => {
    const u = localStorage.getItem('tb_users_v8'); if(u) setDbUsers(JSON.parse(u));
    const p = localStorage.getItem('tb_posts_v8'); if(p) setPosts(JSON.parse(p));
    const t = localStorage.getItem('tb_trans_v8'); if(t) setTransactions(JSON.parse(t));
  }, []);
  useEffect(() => {
    localStorage.setItem('tb_users_v8', JSON.stringify(dbUsers));
    localStorage.setItem('tb_posts_v8', JSON.stringify(posts));
    localStorage.setItem('tb_trans_v8', JSON.stringify(transactions));
  }, [dbUsers, posts, transactions]);

  // Auth Actions
  const handleLogin = (e) => { e.preventDefault(); const u = dbUsers.find(x => x.username === loginForm.username && x.password === loginForm.password); if(u) { setCurrentUser(u); setView(u.role === 'admin' ? 'admin' : 'dashboard'); } else alert('ชื่อผู้ใช้หรือรหัสผ่านไม่ถูกต้อง'); };
  const handleRegister = (e) => { 
    e.preventDefault(); setNotification(null);
    if (!regForm.username || !regForm.password || !regForm.name) return setNotification({ type: 'error', message: 'กรุณากรอกข้อมูลให้ครบ' });
    if (regForm.password !== regForm.confirmPassword) return setNotification({ type: 'error', message: 'รหัสผ่านไม่ตรงกัน' });
    if (dbUsers.find(u => u.username === regForm.username)) return setNotification({ type: 'error', message: 'ชื่อผู้ใช้นี้ถูกใช้ไปแล้ว' });
  ...
  const handleJoinTrip = (post) => {
    if (post.participants.includes(currentUser.name)) return alert("คุณเข้าร่วมทริปนี้แล้ว");
    ...
    setPosts(updatedPosts); alert("เข้าร่วมทริปสำเร็จ! ดูได้ในประวัติของคุณ");
  };

  // Admin & Payment Logic
  const verifyUser = (id, status) => { setDbUsers(dbUsers.map(u => u.id === id ? { ...u, status } : u)); };
  const deleteUser = (id) => { if(confirm('ยืนยันการลบ?')) setDbUsers(dbUsers.filter(u => u.id !== id)); };
  const createPayment = () => {
  ...
  const approvePayment = (transId, userName, postId) => {
    ...
    setPosts(updatedPosts); alert("อนุมัติสำเร็จ! ผู้ใช้ถูกเพิ่มเข้าทริปแล้ว");
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-md p-6 relative animate-in zoom-in duration-200 shadow-2xl overflow-y-auto max-h-[90vh]">
        ...
        {modal.type === 'payment' && (
          <div className="space-y-4 text-center">
            ...
            <div className="border-2 border-dashed border-gray-300 p-6 rounded-xl bg-gray-50 flex flex-col items-center justify-center text-gray-500"><UploadCloud className="w-10 h-10 mb-2"/> <span className="text-sm">จำลองการแนบสลิป</span></div>
